...
class Ch06_Exercise {
	...
	private static void Exercise10() {
		/* (답) b (생성자가 객체를 생성할 때 사용되기는 하지만, 객체를 초기화할 목적으로 사용되는 것이다. 객체를 생성하는 것은 new 연산자이다.)
	...
	private static void Exercise11() {
		/* (답) b (인스턴스메서드에서만 사용 가능하다.)
		 * 클래스 멤버(static 이 붙은 변수나 메서드)에서는 사용할 수 없다.
		 * this 는 인스턴스 자신의 주소를 저장하고 있으며,
		 * 모든 인스턴스메서드에 숨겨진 채로 존재하는 지역변수이다.
	...
	private static void Exercise12() {
		/* (답) c (리턴타입은 오버로딩에 영향을 주지 못한다.)
		 *	   d (매개변수의 이름은 오버로딩에 영향을 주지 못한다.)
		 */
	}
	...
	private static void Exercise13() {
		/* (답) b, c, d
		 *	메서드 이름은 같고 매개변수의 타입이 다르므로 오버로딩이 성립한다.
		 */
	}
	...
	private static void Exercise16() {
		/* (답) a (지역변수는 자동 초기화 되지 않기 때문에 사용하기 전에 반드시 적절한 값으로 초기화를 해주어야 한다.)
		 * 	   e (지역변수는 호출스택(call stack)에 생성된다. 지역변수는 자신이 선언된 블럭이나 메서드가 종료되면 소멸되므로 메모리 부담이 적다.)
		 * 힙(heap) 영역은 인스턴스(인스턴스변수)가 생성되는 영역이다.
		 */
	}
	...
	private static void Exercise17() {
		/* (답) b (호출스택의 제일 위에 있는 메서드가 현재 수행중인 메서드이며,
